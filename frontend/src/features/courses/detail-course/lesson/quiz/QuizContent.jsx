        ...
        <div className="space-y-5">
            {(activeLesson.quizQuestions || []).map((question, qIndex) => (
                <div key={`quiz-question-${qIndex + 1}`} className="rounded-xl border border-slate-200 p-4 dark:border-slate-700">
                    <p className="mb-3 text-sm font-bold">{t('detail.quizQuestionLabel', { index: qIndex + 1, question: question.question })}</p>
            ...
            {quizResult && (
                <div className={`rounded-lg border p-3 text-sm ${quizResult.passed ? 'border-emerald-300 bg-emerald-50 text-emerald-700' : 'border-amber-300 bg-amber-50 text-amber-700'}`}>
                    <p className="font-bold">{t('detail.quizResultTitle')}</p>
                    <p>{t('detail.quizResultScore', { correct: quizResult.correctCount, total: quizResult.total, percentage: quizResult.scorePercent })}</p>
                    <p>{t('detail.quizPassingScore', { passingScore: quizResult.passingScore })}</p>
                </div>
            )}
        </div>
    );
